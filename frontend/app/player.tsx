...
// Paketleri dinamik olarak yükle - kurulu değilse hata vermez
let expoVideoPkg: any = null;
try {
  expoVideoPkg = require("expo-video");
} catch (e) { /* expo-video kurulu değil */ }

// expo-av her zaman import edilir (fallback için gerekli)
import { Video, ResizeMode } from "expo-av";

// Backend URL proxy için
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // YENİ: Akıllı Rewrite destekli tünel proxy adresimiz
...
function PlayerInner({
  ...
  // DÜZELTİLDİ: Olmayan expoPkg kontrolü kaldırıldı
...
function ExpoVideoPlayer({
  ...
  const useEvent = expoVideoPkg?.useEvent;
  ...
  useEffect(() => {
    ...
    if (status === "error" || playerError) {
      ...
      if (!isFailedTriggered.current) {
        ...
        onError(playerError?.message || "Yayın yüklenemedi veya akış koptu");
        timer = setTimeout(() => {
          onFailed();
        }, 1000);
      }
  ...
  }, [status, playerError, onFailed, onError]);
