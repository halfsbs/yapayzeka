...
import {
  api, theme, setToken, M3USource, User, Role, AdminChannel,
...
export default function Admin() {
  ...
  const logout = async () => { await setToken(null); router.replace("/login"); };
      ...
      <LinearGradient colors={["#1A0408", theme.bg]} style={styles.headerGrad}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingHorizontal: 16 }}>
          {tabs.map(t => (
            ...
            >
              <Ionicons name={t.icon} size={14} color={tab === t.key ? "#fff" : theme.textDim} />
        ...
        {cards.map(c => (
          <View key={c.label} style={[styles.statCard]} testID={`stat-${c.label}`}>
            <View style={[styles.statIcon, { backgroundColor: c.c + "22" }]}>
              <Ionicons name={c.icon} size={20} color={c.c} />
            </View>
...
function ChannelsTab({ onToast }: { onToast: (m: string) => void }) {
  ...
  const openEdit = (c: AdminChannel) => { setEditing(c); setEName(c.name); setECat(c.category); setEVip(c.vip); setEHidden(!!c.hidden); };
  const save = async () => {
    if (!editing) return;
    try {
      await api.adminUpdateChannel(editing.id, { name: eName, category: eCat, vip: eVip, hidden: eHidden });
