...
export function isAdultChannel(channel: Channel): boolean {
  return channel.adult === true || channel.category === "+18";
}

export function canWatchChannel(channel: Channel, user: User | null): boolean {
  if (!channel.adult) return true;
  return user?.adult_allowed === true;
}
