...
# Stream URL cache: 5 dakika TTL, max 5000 entry
stream_url_cache = TTLCache(maxsize=5000, ttl=300)
        ...
        if line.startswith("#EXTINF"):
            ...
            if url:
                ...
                if category:
                    ...
                    # Tüm parçaları hem ülke hem tür olarak kontrol et
                    for part in parts:
                        p = part.strip().upper()
                        if p in country_map and not found_country:
                            found_country = country_map[p]
                        if p in type_keywords and not found_type:
                            found_type = type_keywords[p]

                    # Eğer hem ülke hem tür bulunduysa → tür öncelikli (Spor, Sinema, Belgesel...)
                    # Eğer sadece tür bulunduysa → tür
                    # Eğer sadece ülke bulunduysa → ülke
                    if found_type:
                        category = found_type
                    elif found_country:
                        category = found_country
                    else:
                        # Hiçbiri bulunamadıysa, orijinal group-title'ı Türkçeleştirmeye çalış
                        lang_map = {
...
async def list_channels(
    ...
    if not user.get("adult_allowed"):
        query["adult"] = {"$ne": True}
    skip = (page - 1) * limit
    # Dedupe sonrası limit'e ulaşana kadar fazladan çek
    fetch_limit = limit * 5
    ...
    ).skip(skip).limit(fetch_limit).to_list(fetch_limit)
...
async def get_stream(channel_id: str, user: dict = Depends(get_current_user)):
    ...
    cached = stream_url_cache.get(cache_key)
    if cached:
        primary_url, fallback_urls = cached
    else:
        try:
    ...
    stream_url_cache[cache_key] = (primary_url, fallback_urls)
...
async def list_favs(user: dict = Depends(get_current_user)):
    ...
    query = {"norm_name": {"$in": fav_ids}, "hidden": {"$ne": True}}
    if not user.get("adult_allowed"):
        query["adult"] = {"$ne": True}
...
async def admin_list_channels(
    ...
    if q:
        query["name"] = {"$regex": re.escape(q), "$options": "i"}
    skip = (page - 1) * limit
    docs = await db.channels.find(query, {"_id": 0, "stream_url_enc": 0}).skip(skip).limit(limit).to_list(limit)
    total = await db.channels.count_documents(query)
    return {"channels": docs, "page": page, "limit": limit, "total": total}
...
async def seed():
    if not await db.users.find_one({"username": "admin"}):
        admin_pw = os.environ.get("ADMIN_PASSWORD", "busarkilarsenin2000X")
...
async def on_start():
    ...
    try:
        ...
        await db.channels.create_index("hidden")
        await db.channels.create_index("vip")
        await db.channels.create_index("adult")
