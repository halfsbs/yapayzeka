...
# DÜZELTİLDİ: 302 Yönlendirmesi sonrası değişen gerçek video base URL'ini yakalar.
...
async def proxy_stream(channel_id: str, request: Request, segment: Optional[str] = None):
    ...
    # EĞER BU BİR SEGMENT İSTEĞİ İSE (Örn: ?segment=http://iptv.com/1.ts)
    if segment:
        async def segment_generator():
            async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as client:
                async with client.stream("GET", segment, headers=headers) as r:
                    async for chunk in r.aiter_bytes(chunk_size=32768):
                        yield chunk
        return StreamingResponse(segment_generator(), media_type="video/mp2t")

    # EĞER İLK ANA PLAYLIST İSTEĞİ İSE (.m3u8 dosyasını indirip linkleri değiştiriyoruz)
    async with httpx.AsyncClient(follow_redirects=True, timeout=20.0) as client:
        ...
        base_proxy_url = str(request.url).split("?")[0]  # Bizim api proxy adresimiz

        # KRİTİK DÜZELTME: Eğer 302 yönlendirmesi olduysa, base domain için asıl ulaşılan son URL'i baz alıyoruz!
        ...
        for line in m3u8_content.splitlines():
            ...
            # Eğer satır bir yayın linkiyse (http ile başlıyorsa veya relative link ise)
            if line_strip and not line_strip.startswith("#"):
                absolute_ts_url = line_strip
                if not line_strip.startswith("http"):
                    absolute_ts_url = base_stream_dir + line_strip

                # Linki kendimize yönlendiriyoruz: /proxy?segment=http...ts
                new_line = f"{base_proxy_url}?segment={absolute_ts_url}"
