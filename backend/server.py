...
class UserPublic(BaseModel):
    ...
    sports_allowed: bool = False  # ENTEGRE EDİLDİ
...
class Channel(BaseModel):
    ...
    vip: bool = False
    adult: bool = False
    sport: bool = False  # ENTEGRE EDİLDİ
...
class UserSportsUpdate(BaseModel):  # ENTEGRE EDİLDİ
    sports_allowed: bool
...
class ChannelUpdate(BaseModel):
    ...
    sport: Optional[bool] = None  # ENTEGRE EDİLDİ
...
def public_user(u: dict) -> UserPublic:
    return UserPublic(
        ...
        sports_allowed=u.get("sports_allowed", False),  # ENTEGRE EDİLDİ
...
# Çok dilli genişletilmiş filtre sözlükleri
ADULT_KW = ["+18", "adult", "xxx", "porn", "sex", "erotic", "erotik", "beate", "hot", "brazzers", "playboy", "hustler", "vivid", "penthouse", "onlyfans"]
SPORTS_KW = ["sport", "spor", "futbol", "football", "soccer", "match", "bein", "lig", "f1", "motorsport", "basket", "basketball", "nba", "arena", "skysports", "eurosport"]
...
# ---------- M3U Parser (Filtreleme Entegre Edildi) ----------
    ...
    while i < total:
        ...
        if line.startswith("#EXTINF"):
            ...
            if url:
                ...
                # Akıllı çok dilli filtreleme taraması
                check_text = (clean_name or name).lower() + " " + gl
                is_adult = any(k in check_text for k in ADULT_KW)
                is_sport = any(k in check_text for k in SPORTS_KW)
                
                if is_adult and "+18" not in cats:
                    cats.append("+18")
                if is_sport and "Spor" not in cats:
                    cats.append("Spor")
    ...
    try:
        ...
        def _has_override(d: dict) -> bool:
            return bool(
                ...
                or d.get("vip_overridden") or d.get("hidden") or d.get("adult_overridden") or d.get("sport_overridden")
            )
        for c in existing:
        ...
        for c in new_channels:
            ...
            if ov and ov["id"] not in used:
                ...
                if ov.get("sport_overridden"):
                    c["sport"] = ov.get("sport", c.get("sport", False))
                    c["sport_overridden"] = True
...
async def register(request: Request, body: RegisterReq):
    ...
    user = {
        ...
        "blocked": False,
        "adult_allowed": False,
        "sports_allowed": False,  # ENTEGRE EDİLDİ
        "vip_until": None,
        "created_at": datetime.now(timezone.utc),
    }
...
async def get_stream(channel_id: str, user: dict = Depends(get_current_user)):
    ...
    if ch.get("vip") and not is_vip_active(user):
        raise HTTPException(403, "VIP kanal")
    if ch.get("sport") and not user.get("sports_allowed"):
        raise HTTPException(403, "Spor kanali erisimi reddedildi")
    try:
...
async def list_favs(user: dict = Depends(get_current_user)):
    ...
    docs = await db.channels.find(query,
        {"_id": 0, "stream_url_enc": 0, "source_id": 0, "category_overridden": 0, "vip_overridden": 0, "name_overridden": 0, "adult_overridden": 0, "sport_overridden": 0},
    ).to_list(2000)
    seen: dict = {}
    out: List[Channel] = []
    for d in docs:
...
async def admin_update_channel(channel_id: str, body: ChannelUpdate, user: dict = Depends(require_admin)):
    ...
    if body.sport is not None:  # ENTEGRE EDİLDİ
        update["sport"] = body.sport
        update["sport_overridden"] = True
...
@api_router.patch("/admin/users/{user_id}/sports", response_model=UserPublic)  # ENTEGRE EDİLDİ
async def admin_set_sports(user_id: str, body: UserSportsUpdate, user: dict = Depends(require_admin)):
    upd = await db.users.find_one_and_update(
        {"id": user_id}, {"$set": {"sports_allowed": body.sports_allowed}},
        return_document=ReturnDocument.AFTER, projection={"_id": 0, "password": 0},
    )
    if not upd:
        raise HTTPException(404, "Kullanici bulunamadi")
...
async def seed():
    if not await db.users.find_one({"username": "admin"}):
        ...
        await db.users.insert_one({
            ...
            "blocked": False,
            "adult_allowed": True,
            "sports_allowed": True,  # ENTEGRE EDİLDİ
            "vip_until": None,
            "created_at": datetime.now(timezone.utc),
        })
    ...
    if await db.channels.count_documents({}) == 0:
        ...
        for d in demo:
            docs.append({
                ...
                "vip": d["vip"],
                "adult": False,
                "sport": d["category"] == "Spor",  # ENTEGRE EDİLDİ
                "hidden": False,
...
async def on_start():
    ...
    try:
        ...
        await db.channels.create_index("sport")
