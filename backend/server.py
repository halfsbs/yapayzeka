...
async def get_stream(channel_id: str):
    """
    AUTH'SIZ STREAM: Video player'lar native katta çalışır ve JWT token taşıyamazlar.
    Güvenlik: Kanal ID'leri UUID (tahmin edilemez), orijinal URL şifreli.
    VIP/Adult kontrolü kanal listesi çekilirken yapılır (/channels auth'lu).
    """
...
# ============================================================
# STREAM PROXY - M3U8 REWRITE + SEGMENT TUNNELING (AUTH'SIZ)
# ============================================================
import urllib.parse
...
async def proxy_stream(channel_id: str, request: Request, segment: Optional[str] = None):
    """
    AUTH'SIZ PROXY: Video player'lar native katta çalışır ve JWT token taşıyamazlar.
    Bu proxy M3U8 playlist'i rewrite edip segmentleri backend üzerinden tüneller.
    Güvenlik: Kanal ID'leri UUID, orijinal URL Fernet şifreli.
    """
    ch = await db.channels.find_one({"id": channel_id}, {"_id": 0})
    if not ch or ch.get("hidden"):
        raise HTTPException(404, "Kanal bulunamadi")
    ...
    if segment:
        ...
        logger.info(f"[PROXY SEGMENT] {decoded_segment[:100]}...")
                ...
                try:
                    async with client.stream("GET", decoded_segment, headers=headers) as r:
                        if r.status_code == 403:
                            logger.error(f"[PROXY SEGMENT] 403 Forbidden")
                            raise HTTPException(403, "Segment erisimi reddedildi")
                        if r.status_code == 404:
                            logger.error(f"[PROXY SEGMENT] 404 Not Found")
                            raise HTTPException(404, "Segment bulunamadi")
                        if r.status_code != 200:
                            logger.warning(f"[PROXY SEGMENT] {r.status_code}")
                        async for chunk in r.aiter_bytes(chunk_size=32768):
                            yield chunk
                except httpx.ReadTimeout:
                    logger.error(f"[PROXY SEGMENT] Timeout")
                    raise HTTPException(504, "Segment zaman asimina ugradi")
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=httpx.Timeout(60.0, connect=15.0)) as client:
        try:
            ...
            logger.error(f"[PROXY M3U8] Timeout")
            raise HTTPException(504, "IPTV kaynagi zaman asimina ugradi")

        if res.status_code == 403:
            logger.error(f"[PROXY M3U8] 403 Forbidden")
            raise HTTPException(403, "IPTV kaynagi erisimi reddetti")
        ...
        if not m3u8_content.strip().startswith('#EXTM3U'):
            # Belki doğrudan TS stream?
            if '<html' in m3u8_content.lower()[:500]:
                logger.error(f"[PROXY M3U8] HTML dondu")
                raise HTTPException(502, "IPTV kaynagi HTML dondurdu")
            # Doğrudan stream olabilir, redirect et
            logger.info(f"[PROXY M3U8] M3U8 degil, dogrudan stream olabilir. İlk 100: {m3u8_content[:100]}")
            # Fallback: Orijinal URL'yi döndür
            return Response(
        ...
        logger.info(f"[PROXY M3U8] actual={actual_url[:60]}... base={base_stream_dir[:60]}...")
