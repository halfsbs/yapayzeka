...
async def proxy_stream(channel_id: str, request: Request, segment: Optional[str] = None):
    """
    AUTH'SIZ PROXY: Video player'lar (expo-video/expo-av) native katta çalışır
    ve uygulamanın JWT token'ını otomatik taşıyamazlar. Bu yüzden proxy rotası
    auth'siz olmalıdır. Güvenlik: Orijinal stream URL'si şifreli (fernet), 
    kullanıcı hiçbir zaman gerçek IPTV linkini göremez.
    """
    ch = await db.channels.find_one({"id": channel_id}, {"_id": 0})
    if not ch or ch.get("hidden"):
        raise HTTPException(404, "Kanal bulunamadi")
    # NOT: VIP ve adult kontrolü burada yapılamaz çünkü player token taşıyamaz.
    # Bu kontroller kanal listesi çekilirken (auth'lu /channels rotası) yapılır.
    # Kullanıcı zaten sadece uygulama üzerinden erişebilir (kanal ID'leri UUID).
