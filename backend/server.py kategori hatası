...
def parse_m3u(text: str, source_id: str) -> List[dict]:
    ...
    def get_cat(name, clean_name, cat):
        if not cat or cat.lower() == "genel" or cat.lower() == "undefined":
            cat_match = re.match(r'^\[?([A-Za-zÇĞİÖŞÜçğıöşü]{2,10})\]?[\s\|\:\-]+(.*)', clean_name)
            if cat_match:
                extracted = cat_match.group(1).upper()
                mapped = {"TR": "Türkiye", "TUR": "Türkiye", "US": "USA", "UK": "UK",
                          "DE": "Almanya", "FR": "Fransa", "AR": "Arap", "AZ": "Azerbaycan",
                          "RU": "Rusya", "NL": "Hollanda", "PL": "Polonya", "IT": "İtalya",
                          "ES": "İspanya", "SPORT": "Spor", "SPORTS": "Spor",
                          "MOVIE": "Sinema", "MOVIES": "Sinema", "DOC": "Belgesel"}
                cat = mapped.get(extracted, extracted.title())
            else:
                cat = "Genel"
        return cat
    ...
    while i < total:
        ...
        if line.startswith("#EXTINF"):
            ...
            if url:
                ...
                cat = attrs.get("group-title")
                cat = get_cat(name, clean_name, cat)
